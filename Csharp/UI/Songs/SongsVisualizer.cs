...
public partial class SongsVisualizer : ScrollContainer
{
    ...
    private void UpdateSongs(in Playlist playlist)
    {
        ...
        if(song_displays.Count != target_displays) // NOTE: Is only true if there is more songs added (only when previous count was below max_displays), or the size changed.
        {
            ...
            else // Add new
            {
                for(int i = song_displays.Count; i < target_displays; i++)
                {
                    ...
                    song_displays[i].Init(song, data, playlist.type, GetCover(playlist, song));
                }
            }
        }
        ...
        if(shift != 0 && ScrollVertical > top_card_size_y + SINGLE_BUTTON_SIZE || a_shift < song_displays.Count) // NOTE: Is only true if there has been any scrolling, and past the beginning.
        {
            ...
            for(int i = 0; i < a_shift; i++)
            {
                ...
                SongData data = GetSongData(song, playlist);
                song_displays[index].Init(song, data, playlist.type, GetCover(playlist, song));
            }
        }
    ...
    public void UpdateSong(int index, in SongData song_data, in Texture2D cover)
    {
        ...
        song_displays[index - first_song].Init(index, song_data, GetPlaylist().type, cover);
    }
