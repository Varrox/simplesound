...
public class MainParser
{
    ...
    /*public static void ParseDFile<T>(string path, out T output_object, out List<string> tags) where T : new()
    {
            ...
            else if (new_line)  // New Line
            {
                if (text[i] == '#') // Tags
                {
                    for (int j = i + 1; j < text.Length; j++) 
                    {
                        if (text[j] == '\n')
                        {
                            tags.Add(text.Substring(i + 1, j - 1).Trim());
                            i = j;
                            break;
                        }
                    }
                ...
                if (!string.IsNullOrWhiteSpace($"{text[i]}") && zeroed) new_line = false;
            }
            else if (trimmed_start_line.StartsWith(typeof(T).Name))
            {
                for (int j = i + 1; j < text.Length; j++)
                {
                    line.line = j;
                    string trimmed_line = ParsingTools.FormatCode(text[j]);
                    ...
                    if (trimmed_line.Length > 0)
                    {
                        ...
                        if (ParsingTools.IsTypeEnumeral(field.FieldType))
                        {
                            (int, int) ending = GetEnding(text, j, text[j].Length - var.Length, '}');
